...
## Note
本应用兼容 [小黑盒 1.3.393](https://github.com/Mrmiaomrzh/BetterHeybox/releases/download/v0.2.0/heybox_1.3.393.apk) 与 **1.3.394** 两个版本（运行时自动识别），
其他版本出现的问题不会进行处理
...
- **自动完成每日分享任务**：开启后自动分享**自定义帖子**完成小黑盒每日「分享帖子」任务，
  不拦截 QQ 分享入口（只触发任务自身分享成功回调）；需先在「自定义分享帖子」中配置要分享的帖子链接
  （一行一个，支持 `api.xiaoheihe.cn/...web/share?link_id=`、`xiaoheihe.cn`、`heybox://` 链接）
- **图片系统分享**：图片长按分享改为优先保存到系统相册（可真正查看/分享），自动识别图片真实格式
